...
export async function verifyNetwork(ip: string, branchId: string | null) {
  ...
  if (configs.length === 0) return { ok: true, reason: 'no_config_open' }; // for initial setup
...
export async function verifyGeofence(lat:number,lng:number,branchId:string|null){
  if (!branchId) return { ok: true, distance: 0 };
  const branch = await prisma.branch.findUnique({ where: { id: branchId }});
  if (!branch || branch.latitude == null || branch.longitude == null) return { ok: true, distance: 0 };
...
export async function punchIn(opts: {
  ...
  ip: string; device: any;
  selfieVerified: boolean; selfieConfidence?: number; selfieFileId?: string;
}) {
  const employee = await prisma.employee.findUnique({ where: { id: opts.employeeId }, include:{ shift:true } });
  if (!employee) throw new Error('Employee not found');
  if (employee.employmentStatus === 'TERMINATED') throw new Error('Terminated');
  ...
  if (existing?.punchInAt) throw new Error('Already punched in today');

  const net = await verifyNetwork(opts.ip, employee.branchId);
  if (!net.ok) throw new Error('Punch In is only available from an authorized Coffee Bun office location.');

  const geo = await verifyGeofence(opts.lat, opts.lng, employee.branchId);
  if (!geo.ok) throw new Error(`Outside geofence: ${geo.distance}m > ${geo.allowedRadius}m`);

  if (!opts.selfieVerified) throw new Error('Selfie verification failed');
...
export function calcWorked(punchIn: Date, punchOut: Date, breakMinutes=60) {
...
export async function punchOut(opts: {
  employeeId: string;
  lat: number; lng: number; ip: string; device: any;
}) {
  const dateStr = todayIST();
  const date = new Date(dateStr+'T00:00:00.000Z');
  const att = await prisma.attendance.findUnique({ where: { employeeId_date: { employeeId: opts.employeeId, date }}});
  if (!att || !att.punchInAt) throw new Error('Not punched in');
  if (att.punchOutAt) throw new Error('Already punched out');
