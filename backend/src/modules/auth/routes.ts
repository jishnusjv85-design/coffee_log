...
router.post('/login', async (req, res) => {
  ...
  if (user.lockedUntil && user.lockedUntil > new Date()) return res.status(423).json({ error: 'Account locked' });
...
router.post('/refresh', async (req, res) => {
  ...
  try {
    const decoded: any = verifyRefresh(token);
    ...
    if (!rt || rt.expiresAt < new Date()) return res.status(401).json({ error: 'Expired' });
    // rotate
...
router.post('/change-password', requireAuth, async (req, res) => {
  ...
  if (!p.success) return res.status(400).json({ error: 'Weak password' });
