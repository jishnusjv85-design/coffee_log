...
async function main(){
  ...
  for(const r of roles){
    await prisma.role.upsert({ where:{ name: r as any }, update:{}, create:{ name: r as any, description: r }});
  }
  // permissions
  const permKeys = [
  ...
  async function ensureUser(email:string, employeeCode:string, fullName:string, roleName:'SUPER_ADMIN'|'ADMIN'|'EMPLOYEE', password:string){
